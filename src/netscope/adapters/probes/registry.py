...
class ProbeRegistry:
    ...
    Registered by default: ICMP, DNS, HTTP -- the three adapters that
    exist today (netscope.adapters.probes.icmp_adapter/dns_adapter/
    http_adapter, added in TASK-007). TCP, TLS, and TRACEROUTE are valid
    ProbeType members but have no adapter implementation yet (per
    future-roadmap.md TASK-016/017/019) -- looking one of those up
    raises ProbeNotRegisteredError rather than returning None or an
    incorrect probe, so the failure is loud and immediate rather than a
    confusing AttributeError three calls later.
    """
    ...
    def register(self, probe_type: ProbeType, probe: Probe) -> None:
        ...
        Exists so a future probe (e.g. a TCP adapter, TASK-016) or a
